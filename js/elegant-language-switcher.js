 ...
 * Version: 2.0.0 - Vanilla JS (no jQuery dependency)
...
(function () {
  ...
  function buildMobileSwitcher() {
    ...
    var existing = document.querySelectorAll('.mobile-language-switcher');
    existing.forEach(function (el) { el.remove(); });

    // Detect current language
    var activeLangEl = document.querySelector('.language-switcher .active-lang') ||
                       document.querySelector('.language-switcher .lang-option.active');
    var currentLang = activeLangEl ? activeLangEl.textContent.trim() : 'EN';

    // Build options array from desktop switcher
    var optionEls = document.querySelectorAll('.language-switcher .lang-option');
    var langOptions = [];
    ...
    // Create mobile switcher element
    var wrapper = document.createElement('div');
    wrapper.innerHTML = generateMobileHTML(currentLang, langOptions);
    var dropdown = wrapper.firstElementChild;

    // Insert before .menu-toggle
    var menuToggle = document.querySelector('.menu-toggle');
    if (menuToggle && menuToggle.parentNode) {
      menuToggle.parentNode.insertBefore(dropdown, menuToggle);
    }
  ...
  function generateMobileHTML(current, options) {
    var optionsHTML = options
      .map(function (option) {
        return '<a href="' + option.url + '" class="' + (option.active ? 'active' : '') + '" role="menuitem">' + option.lang + '</a>';
      })
      .join('');

    return '<div class="mobile-language-switcher">' +
  ...
  function bindMobileEvents(root) {
    var toggle = root.querySelector('.mobile-lang-toggle');
    var options = root.querySelector('.mobile-lang-options');
    ...
    var optionLinks = options.querySelectorAll('a');
    optionLinks.forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        if (currentLangEl) currentLangEl.textContent = selectedLang;

        closeDropdown(toggle, options, arrow);

        document.body.classList.add('page-transitioning');
        setTimeout(function () { window.location.href = targetUrl; }, 200);
      });
    });
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && options.style.display === 'block') {
        closeDropdown(toggle, options, arrow);
        toggle.focus();
      }
    });
